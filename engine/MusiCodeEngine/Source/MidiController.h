...
public:
    ...
    /** 
     * 設定特定軌道的輸入源
     * @param trackIndex 軌道索引 (0-based)
     * @param deviceName 設備名稱 (如 "iCON iKeyboard 4", "All MIDI Ins")
     */
    bool setTrackInput(int trackIndex, const juce::String& deviceName);

    /** 更新設備列表 (當 AudioDeviceSelector 變更時呼叫) */
    void refreshDevices();

    /** 註冊一個回呼，當有 MIDI 訊號強度更新時通知 (30Hz) */
    void setSignalLevelCallback(std::function<void(int trackIndex, float level)> callback)
    {
...
private:
    ...
    std::function<void(int trackIndex, float level)> signalLevelCallback;

    // 快取上一次的訊號強度，減少 IPC 負擔
    std::vector<float> lastSignalLevels;
